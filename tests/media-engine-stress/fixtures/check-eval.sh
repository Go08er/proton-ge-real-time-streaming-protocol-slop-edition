...
die() {
    printf 'fixture eval: %s\n' "$*" >&2
    exit 1
}

[[ $# -ge 1 && $# -le 2 ]] || die 'usage: check-eval.sh /nix/store/<pinned-nixpkgs-source> [smoke|full]'
nixpkgs_path=$1
profile=${2:-smoke}
...
nix-instantiate "$script_dir/default.nix" \
    --argstr nixpkgsPath "$nixpkgs_path" \
    --argstr profile "$profile" \
    --show-trace >/dev/null
printf 'evaluated fixture profile %s against %s\n' "$profile" "$nixpkgs_path"
